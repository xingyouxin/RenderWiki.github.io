...
![](http://latex.codecogs.com/svg.latex?p_{n-1}=M_{n-1}M_{n}^{-1}p_n\\
其中M为view-projection矩阵)
...
![](http://latex.codecogs.com/svg.latex?f_n(p)=\alpha \cdot s_n(p) + (1 - \alpha)\cdot f_{n-1}(\pi(p))\\
f_n是第n帧的输出\\
\alpha是融合参数\\
\pi(\cdot)是当前位置在前一帧中的投影\\
当前多数做法:\alpha取一个固定的值)
...
![](http://latex.codecogs.com/svg.latex?\alpha=\frac{1}{N_t(p)}\\
N_t(p)是每个像素积累的采样点数目)
...
![](http://latex.codecogs.com/svg.latex?w(c)=\frac{1}{1+L(c)}\\
L(c)是颜色c的luma通道\\
该权重在blend时乘在当前帧的sample和历史帧的sample前)
...
![](http://latex.codecogs.com/svg.latex?\varepsilon_n(p)=B_3\ast(f_{n-1}-s_n)(p)\\
B3是一个半径为3的box\ filter)
...
![](http://latex.codecogs.com/svg.latex?公式代码C_{min}=\mu-\gamma\sigma\\
C_{max}=\mu+\gamma\sigma\\
\mu是当前帧当前像素临近像素的颜色的平均值\\
\sigma是当前帧当前像素临近像素的颜色的方差\\
\gamma的值在0.75-1.25之间)
