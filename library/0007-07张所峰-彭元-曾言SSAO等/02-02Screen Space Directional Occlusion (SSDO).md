...
## 1.SSDO（screenspace directional occlusion）
...
如图1.6所示，A的深度比z1更远，因此被错误地分类为遮挡，而B的深度尽管比最近深度更近，却被错误地分类为可见，此时P点的直接光照就被错误计算。在得到前n个深度后，能得知A并不处于z1和z2深度之间，因此被分类为可见；在B方向上多采样，就能找到遮挡。
